...
data.products.forEach((product) => {
  cardsRow.innerHTML += `<div class="col-lg-3 col-md-6 col-sm-12">
    <div class="card">
        <div data-id=${product.id}  class=""><i class=" heart   fa-regular fa-heart" style="color: #ff0000;"></i></div>
        <img class="card-img-top" height="240" src="${product.imgSrc}" alt="${product.name
    }">
  <div class="card-body">
            <h5 class="card-title">${product.name}</h5>
            <h5>${product.hasDiscount()
  ...
  hearts.forEach(heart => {
    heart.addEventListener("click", function() {
      ...
      if (this.classList.contains("fa-regular"," fa-heart")) {
        this.classList.replace("fa-regular","fa-solid")
      }else{
        this.classList.replace("fa-solid","fa-regular")
      }
      let productIdx = this.parentElement.getAttribute("data-id")
      wishlistS.push({id:productIdx})
